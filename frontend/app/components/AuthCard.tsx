...
// Define separate form data types
...
export default function AuthCard({ type }: AuthCardProps) {
  ...
  // Use the correct schema and type based on type prop
  ...
  const handleFormSubmit = async (formData: LoginFormData | RegisterFormData) => {
    ...
    const payload = isRegister
      ? (formData as RegisterFormData)  // has name
      : (formData as LoginFormData);     // no name
    ...
    try {
      ...
      if (isRegister) {
        ...
        // Save user's name (or email if name is not available)
        ...
        <div className="bg-white w-full max-w-md rounded-xl shadow-lg p-8">
            ...
            {isRegister && (
              <div>
                <input
                  ...
                  {...register("name")}
                />
                {isRegister && "name" in errors && errors.name && (
          ...
          <p className="text-sm text-center mt-5">
            ...
            <Link
                ...
                prefetch={false}          // ← prevents preloading
>
